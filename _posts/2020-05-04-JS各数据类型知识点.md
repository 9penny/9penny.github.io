...
当对象中还包含对其他对象的引用, 我们就要深拷贝了

```JavaScript
//很多库都有现成的实现, 比如lodash中的_.cloneDeep(obj)
//也可以自己造轮子, 这里使用递归

//不包含函数
let obj = {
    person: {name: "Lim", age: 30},
    count: 0
};

function deepCloneWithoutFunction(obj) {
    let newObj = {};
...
JavaScript的`this`是自由的, 它的值在调用时被计算出来
...
通常构造器没有`return`语句, 它只是把必要的东西写入`this`
...
`let descriptor = Object.getOwnPropertyDescriptor(obj, propertyName);`  这个方法可以查看属性的属性标志
...
修改标志可以用`Object.defineProperty(obj, properName, descriptor)`
...
修改多个标志可以用`Object.defineProperties`
...
##### 利用Object.getOwnPropertyDescriptors克隆对象的所有属性标识符
...
let john = new User("John", new Date(1999,01,01));
```
...
`Symbol`表示唯一的标识符, 可以给他一个备注, 但是两个相同备注的`Symbol`仍然不相等
...
Symbol可以创建对象的"隐藏"属性, 代码的其他部分都不会意外的访问或者重写这些属性, 例如使用第三方代码的`user`对象时
...
这些Symbol可以用于微调对象的各个方面, 可以在一个表中查询: [Symbol表](https://tc39.github.io/ecma262/#sec-well-known-symbols)
